...
namespace til
{
    ...
    {
        if (!wstr.empty() && is_leading_surrogate(wstr[idx++]))
        {
            if (idx != wstr.size() && is_trailing_surrogate(wstr[idx]))
            {
                ++idx;
            }
        }
    ...
    {
        if (!wstr.empty() && is_trailing_surrogate(wstr[--idx]))
        {
            if (idx != 0 && is_leading_surrogate(wstr[idx - 1]))
            {
                --idx;
            }
        }

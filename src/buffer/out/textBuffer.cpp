...
void TextBuffer::Reflow(TextBuffer& oldBuffer, TextBuffer& newBuffer, const Viewport* lastCharacterViewport, PositionInformation* positionInfo)
{
    ...
    const auto mutableViewportTop = positionInfo ? positionInfo->mutableViewportTop : til::CoordTypeMin;
    const auto visibleViewportTop = positionInfo ? positionInfo->visibleViewportTop : til::CoordTypeMin;
    ...
    for (; oldY < oldHeight; ++oldY)
    {
        ...
        do
        {
            ...
            if (newX >= newWidth)
            {
                newBuffer.GetRowByOffset(newY).SetWrapForced(true);
                newX = 0;
                newY++;
            }
            ...
            {
                newBuffer.GetRowByOffset(newY).Reset(newBuffer._initialAttributes);
            }
            ...
            {
                newCursorPos = { oldCursorPos.x - oldX + newX, newY };
            }
    ...
    {
        newBuffer.GetRowByOffset(newCursorPos.y).SetWrapForced(true);
        newCursorPos.x = 0;
        newCursorPos.y++;
        ...
        {
            newBuffer.GetRowByOffset(newCursorPos.y).Reset(newBuffer._initialAttributes);
            newX = 0;
            newY = newCursorPos.y + 1;
        }
    }
    ...
    if (newY > newHeight)
    {
        ...
        // newCursorPos.y might be greater than newHeight, but the cursor might still not
        // be in the last row of the buffer (= newCursorPos.y might be smaller than newY).
        // What this line does is similar to `newCursorPos.y % newHeight` but we do it relative to the actual end
        // of the buffer, so that a cursor 10 lines above the end of the buffer stays 10 lines above the end.
        newCursorPos.y = newHeight - (newY - newCursorPos.y) % newHeight;
    }

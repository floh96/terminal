...
// Return Value:
void AdjustCursorPosition(SCREEN_INFORMATION& screenInfo, DWORD flags, _In_ til::point coordCursor, _Inout_opt_ til::CoordType* scrollY)
{
    ...
    if (coordCursor.y >= bufferSize.height)
    {
        ...
        if (scrollY)
        {
            *scrollY += coordCursor.y - bufferSize.height - 1;
        }
    ...
    if (WI_IsFlagSet(flags, WC_KEEP_CURSOR_VISIBLE))
    {
        screenInfo.MakeCursorVisible(coordCursor);
    }

    LOG_IF_FAILED(screenInfo.SetCursorPosition(coordCursor, WI_IsFlagSet(flags, WC_KEEP_CURSOR_VISIBLE)));
}

// As the name implies, this writes text without processing its control characters.
static void _writeCharsLegacyUnprocessed(SCREEN_INFORMATION& screenInfo, const DWORD flags, const std::wstring_view& text, til::CoordType& psScrollY)
{
    ...
    while (!state.text.empty())
    {
        ...
        AdjustCursorPosition(screenInfo, flags, cursorPosition, &psScrollY);
    }
}
...
// - pwchBuffer - Pointer to string to write.
// - cch - number of chars to write.
...
til::CoordType WriteCharsLegacy(SCREEN_INFORMATION& screenInfo, DWORD flags, const std::wstring_view& str)
{
    ...
    if (cursor.IsDelayedEOLWrap() && wrapAtEOL)
    {
        ...
        if (delayed == pos)
        {
            pos.x = 0;
            pos.y++;
            AdjustCursorPosition(screenInfo, flags, pos, &scrollY);
        }
    }
    ...
    {
        _writeCharsLegacyUnprocessed(screenInfo, flags, { it, end }, scrollY);
        it = end;
    }
    ...
    while (it != end)
    {
        ...
        {
            _writeCharsLegacyUnprocessed(screenInfo, flags, { it, nextControlChar }, scrollY);
            it = nextControlChar;
        }
        ...
        for (; it != end && !IS_GLYPH_CHAR(*it); ++it)
        {
            ...
            {
            case UNICODE_NULL:
                if (WI_IsFlagSet(flags, WC_INTERACTIVE))
                {
                    break;
                }
                _writeCharsLegacyUnprocessed(screenInfo, flags, { &tabSpaces[0], 1 }, scrollY);
                continue;
            case UNICODE_BELL:
                if (WI_IsFlagSet(flags, WC_INTERACTIVE))
                {
            ...
            case UNICODE_BACKSPACE:
            {
                ...
                assert(WI_IsFlagClear(flags, WC_INTERACTIVE));
                auto pos = cursor.GetPosition();
            ...
            case UNICODE_TAB:
            {
                ...
                _writeCharsLegacyUnprocessed(screenInfo, flags, { &tabSpaces[0], tabCount }, scrollY);
                continue;
            }
            ...
            if (WI_IsFlagSet(flags, WC_INTERACTIVE) && IS_CONTROL_CHAR(*it))
            {
                const wchar_t wchs[2]{ L'^', static_cast<wchar_t>(*it + L'@') };
                _writeCharsLegacyUnprocessed(screenInfo, flags, { &wchs[0], 2 }, scrollY);
            }
            else
            {
                ...
                if (result == 1)
                {
                    _writeCharsLegacyUnprocessed(screenInfo, flags, { &wch, 1 }, scrollY);
                }
            }
        }
    }
    ...
    return (scrollY + finalCursorPos.y - initialCursorPos.y) * textBuffer.GetSize().Width() + finalCursorPos.x - initialCursorPos.x;
}
...
try
{
    ...
    {
        WriteCharsLegacy(screenInfo, 0, str);
    }
    else
    {
